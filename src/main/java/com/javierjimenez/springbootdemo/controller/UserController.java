...
import com.javierjimenez.springbootdemo.errors.EntityNotFoundException;
...
class UserController {
    ...
    @PutMapping("/{id}")
    @PreAuthorize("!hasAuthority('USER') || (authentication.principal == @userRepository.findById(#id).orElse(new com.javierjimenez.springbootdemo.entity.User()).email)")
    ...
    @PreAuthorize("!hasAuthority('USER') || (#oldPassword != null && !#oldPassword.isEmpty() && authentication.principal == @userRepository.findById(#id).orElse(new com.javierjimenez.springbootdemo.entity.User()).email)")
